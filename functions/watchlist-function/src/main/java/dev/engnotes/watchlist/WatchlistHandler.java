...
public class WatchlistHandler {
    ...
    static final String MODULE_LABEL = "financial-watchlist";
    ...
    public Function<WatchlistRequest, WatchlistResponse> watchlist(
        ...
        return request -> {
            try (var ctx = RequestContext.begin(MODULE_LABEL, request.correlationId())) {
                ...
                // Consent gate (spec decision 5): no active consent -> no watchlist interaction at
                ...
                return switch (request.operation()) {
                    case ADD -> {
                        // Spec s11 erasure step 1: refuse new watchlist rows while erasure is in
                        // flight. Scoped to ADD only, not the consent check above: REMOVE and LIST
                        // stay allowed. The "deletion pending" prefix keeps QueryStack's
                    ...
                    case REMOVE -> {
                        ...
                        // QueryStack's watchlist DELETE 409 mapping in sync; changing it here
                        // requires the matching change in QueryStack.
