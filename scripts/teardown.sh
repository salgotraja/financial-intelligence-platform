#!/usr/bin/env bash
# Destroy every ephemeral stack (Query, Ingestion, Security), keeping ONLY DataStack.
# DataStack holds the deletion-protected, permanent DPDP audit table and is NEVER a destroy target.
# cdk resolves the safe destroy order from stack dependencies. Idempotent.
...
log "Destroying $STACK_QUERY, $STACK_INGESTION, $STACK_SECURITY (only Data retained)..."
...
run bash -c "cd '$REPO_ROOT/infrastructure' && $CDK destroy '$STACK_QUERY' '$STACK_INGESTION' '$STACK_SECURITY' --context env=$ENV --force --output '$CDK_OUT'"
